...
const DigitBotPanel = () => {
  ...
  const [strategy, setStrategy] = React.useState("OVER");
  ...
  const [stopWin, setStopWin] = React.useState(10);
    ...
    <Card className="mt-4">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="text-xs">Stop Win</label>
            <input
              type="number"
              min="0"
              value={stopWin}
        ...
        <div className="rounded-md border p-3 text-sm">
          ...
          <div>Target: {targetDigit}</div>
          <div>Stake: {stake}</div>
          <div>Max Trades: {maxTrades}</div>
        </div>
        ...
        <div className="text-xs text-muted-foreground">
          Demo mode — the bot does not place real trades.
        </div>
...
const DigitBotPanel = () => {
  const [running, setRunning] = React.useState(false);
  const [strategy, setStrategy] = React.useState<"OVER" | "UNDER">("OVER");
  const [targetDigit, setTargetDigit] = React.useState(5);
  const [stake, setStake] = React.useState(1);
  const [maxTrades, setMaxTrades] = React.useState(10);
  const [stopLoss, setStopLoss] = React.useState(10);
  ...
  const runBot = () => {
    ...
    // Replace this section with your real tick/trading API when connected.
    ...
    setTrades((current) => [...current, newTrade]);

    const newProfit = totalProfit + profit;

    if (
      trades.length + 1 >= maxTrades ||
      newProfit <= -Math.abs(stopLoss) ||
      newProfit >= Math.abs(stopWin)
    ) {
      setRunning(false);
    } else {
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select
              value={strategy}
              onChange={(e) =>
                setStrategy(e.target.value as "OVER" | "UNDER")
              }
          ...
          <div>
            <label className="text-sm font-medium">Target digit</label>
          ...
          <div>
            ...
            <input
              ...
              value={stake}
              onChange={(e) =>
                setStake(Math.max(0.01, Number(e.target.value)))
              }
              className="mt-1 w-full rounded-md border bg-background px-3 py-2"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={maxTrades}
              onChange={(e) =>
                setMaxTrades(Math.max(1, Number(e.target.value)))
              }
              className="mt-1 w-full rounded-md border bg-background px-3 py-2"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={stopLoss}
              onChange={(e) =>
                setStopLoss(Math.max(0, Number(e.target.value)))
              }
              className="mt-1 w-full rounded-md border bg-background px-3 py-2"
            />
          </div>
          ...
          <div>
            <label className="text-sm font-medium">Stop win</label>
            ...
            <input
              ...
              value={stopWin}
              onChange={(e) =>
                setStopWin(Math.max(0, Number(e.target.value)))
              }
              className="mt-1 w-full rounded-md border bg-background px-3 py-2"
            />
          </div>
              ...
              {trades
                ...
                .map((trade) => (
                  ...
                  >
                    ...
                    <span>
                      Digit <strong>{trade.digit}</strong>
                    </span>
